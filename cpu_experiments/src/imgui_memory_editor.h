...
// The code assume a mono-space font for simplicity! If you don't use the
// default font, use ImGui::PushFont()/PopFont() to switch to a mono-space font
// before caling this.
//
// Usage:
...
struct MemoryEditor {
  ...
  void DrawContents(void* mem_data_void_ptr,
                    size_t mem_size,
                    size_t base_display_addr = 0x0000) {
    ...
    if (OptShowDataPreview) {
      ...
      if (has_value)
        DisplayPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType,
  ...
  static void* EndianessCopyBigEndian(void* _dst,
                                      void* _src,
                                      size_t s,
  ...
  static void* EndianessCopyLittleEndian(void* _dst,
                                         void* _src,
                                         size_t s,
                                         int is_little_endian) {
    if (is_little_endian) {
  ...
  void DisplayPreviewData(size_t addr,
                          const u8* mem_data,
                          size_t mem_size,
                          DataType data_type,
